  ...
  switch (action.type) {
    ...
    case actions.ADD_ITEM: {
      ...
      const carbonFootprint = updatedItems.reduce(
        (total, item) => total + 
          ((item.product.carbonFootprint || 0) * item.quantity), 0
      );
...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    const saveCartToStorage = () => {
      try {
        localStorage.setItem('greenCart', JSON.stringify({
          items: state.items,
  ...
  // Cart actions
  const fetchCart = async (userId = 'guest-user') => {
  ...
  const addToCart = async (product, quantity = 1, userId = 'guest-user') => {
    ...
    try {
      // Green practice: only make API call if online to save bandwidth
      if (navigator.onLine) {
  ...
  const removeFromCart = async (productId, userId = 'guest-user') => {
    ...
    try {
      // Green practice: only make API call if online
      if (navigator.onLine) {
        await axios.delete(`/api/cart/remove/${productId}?userId=${userId}`);
      }
    } catch (error) {
  ...
  const toggleGreenDelivery = async (userId = 'guest-user') => {
    ...
    try {
      // Green practice: only make API call if online
      if (navigator.onLine) {
        await axios.patch('/api/cart/green-options', {
          userId,
          greenDelivery: !state.greenDelivery
        });
      }
    } catch (error) {
      console.error('Error updating green delivery option:', error);
    }
  };
  ...
  const toggleCarbonOffset = async (userId = 'guest-user') => {
    ...
    try {
      ...
      console.error('Error updating carbon offset option:', error);
    }
  };
